 ...
 */
define('LINGUA_POTX_API_CURRENT', 7);

/**
 * Silence status reports.
 */
define('LINGUA_POTX_STATUS_SILENT', 0);

/**
 * Drupal message based status reports.
 */
define('LINGUA_POTX_STATUS_MESSAGE', 1);
 ...
 */
define('LINGUA_POTX_STATUS_CLI', 2);
 ...
 */
define('LINGUA_POTX_STATUS_STRUCTURED', 3);
 ...
 */
define('LINGUA_POTX_BUILD_CORE', 0);
 ...
 */
define('LINGUA_POTX_API_7', 7);

/**
 * When no context is used. Makes it easy to look these up.
 */
define('LINGUA_POTX_CONTEXT_NONE', NULL);

/**
 * When there was a context identification error.
 */
define('LINGUA_POTX_CONTEXT_ERROR', FALSE);
 ...
 */
function lingua_potx_process_file( $file_path, $strip_prefix = 0, $save_callback = 'lingua_potx_save_string', $version_callback = 'lingua_potx_save_version', $api_version = LINGUA_POTX_API_CURRENT) {
  ...
  lingua_potx_find_version_number($code, $file_name, $version_callback);

  // The .info files are not PHP code, no need to tokenize.
  if ($name_parts['extension'] == 'info') {
    lingua_potx_find_info_file_strings($file_path, $file_name, $save_callback, $api_version);
    return;
  }
  elseif ($name_parts['extension'] == 'js' && $api_version > LINGUA_POTX_API_5) {
    // @todo: D7 context support.
    lingua_potx_parse_js_file($code, $file_name, $save_callback);
  }
      ...
      if (is_array($token)) {
        $token[] = $line_number;
         // Fill array for finding token offsets quickly.
          /** Lingua START modifications */
          $src_tokens = apply_filters('lingua_scan_gettext_translate_functions', array(
  ...
  unset($raw_tokens);
    ...
    if ( !empty( $src_tokens ) ) {
        foreach ( $src_tokens as $tk ) {
            lingua_potx_find_t_calls_with_context( $file_name, $save_callback, $tk );
        }
    }
    /** Lingua END modifications */
}
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_build_files( $string_mode = LINGUA_POTX_STRING_RUNTIME, $build_mode = LINGUA_POTX_BUILD_SINGLE, $force_name = 'general', $save_callback = 'lingua_potx_save_string', $version_callback = 'lingua_potx_save_version', $header_callback = '_potx_get_header', $template_export_langcode = NULL, $translation_export_langcode = NULL, $api_version = LINGUA_POTX_API_CURRENT) {
  global $_potx_store;
  ...
  foreach ($strings as $string => $string_info) {
    foreach ($string_info as $context => $file_info) {
      ...
      if ($build_mode == LINGUA_POTX_BUILD_SINGLE) {
      ...
      elseif (strpos($comment, '.info')) {
        ...
        $file_name = ($build_mode == LINGUA_POTX_BUILD_CORE ? 'general' : str_replace('.info', '.module', $file_name));
      }
      ...
      if (strpos($string, "\0") !== FALSE) {
        ...
        if (isset($translation_export_langcode)) {
          $output .= lingua_potx_translation_export($translation_export_langcode, $singular, $plural, $api_version);
        }
        else {
          $output .= "msgstr[0] \"\"\n";
          $output .= "msgstr[1] \"\"\n";
        ...
        $output .= "msgid \"$string\"\n";
        if (isset($translation_export_langcode)) {
          $output .= lingua_potx_translation_export($translation_export_langcode, $string, NULL, $api_version);
        }
        else {
          $output .= "msgstr \"\"\n";
        }
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_translation_export( $translation_export_langcode, $string, $plural = NULL, $api_version = LINGUA_POTX_API_CURRENT) {
  ...
  $language_column = $api_version > LINGUA_POTX_API_5 ? 'language' : 'locale';
  $language_table  = $api_version > LINGUA_POTX_API_5 ? 'languages' : 'locales_meta';
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_get_header( $file, $template_export_langcode = NULL, $api_version = LINGUA_POTX_API_CURRENT) {
  // We only have language to use if we should export with that langcode.
  $language = NULL;
  if (isset($template_export_langcode)) {
    $language = db_query($api_version > LINGUA_POTX_API_5 ? "SELECT language, name, plurals, formula FROM {languages} WHERE language = :langcode" : "SELECT locale, name, plurals, formula FROM {locales_meta} WHERE locale = :langcode", array( ':langcode' => $template_export_langcode))->fetchObject();
  }
 ...
 */
function lingua_potx_write_files( $http_filename = NULL, $content_disposition = 'inline') {
  global $_potx_store;
 ...
 */
function lingua_potx_format_quoted_string( $str) {
  $quo = substr($str, 0, 1);
  $str = substr($str, 1, -1);
  if ($quo == '"') {
 ...
 */
function lingua_potx_marker_error( $file, $line, $marker, $ti, $error, $docs_url = NULL) {
  global $_potx_tokens;
  ...
  lingua_potx_status('error', $error, $file, $line, $marker .'('. $tokens, $docs_url);
}
 ...
 */
function lingua_potx_status( $op, $value = NULL, $file = NULL, $line = NULL, $excerpt = NULL, $docs_url = NULL) {
  static $mode = LINGUA_POTX_STATUS_CLI;
  ...
  switch ($op) {
    ...
    case 'status':
      ...
      $location_info = '';
      if (($mode != LINGUA_POTX_STATUS_STRUCTURED) && isset($file)) {
        if (isset($line)) {
          if (isset($excerpt)) {
            $location_info = lingua_t('At %excerpt in %file on line %line.', array( '%excerpt' => $excerpt, '%file' => $file, '%line' => $line));
          }
          else {
            $location_info = lingua_t('In %file on line %line.', array( '%file' => $file, '%line' => $line));
          }
        }
        else {
          if (isset($excerpt)) {
            $location_info = lingua_t('At %excerpt in %file.', array( '%excerpt' => $excerpt, '%file' => $file));
          }
          else {
            $location_info = lingua_t('In %file.', array( '%file' => $file));
          }
        }
      }
      ...
      $read_more = '';
      if (($mode != LINGUA_POTX_STATUS_STRUCTURED) && isset($docs_url)) {
        $read_more = ($mode == LINGUA_POTX_STATUS_CLI) ? lingua_t('Read more at @url', array( '@url' => $docs_url)) : lingua_t('Read more at <a href="@url">@url</a>', array( '@url' => $docs_url));
      }

      // Error message or progress text to display.
      switch ($mode) {
        case LINGUA_POTX_STATUS_MESSAGE:
            /** Lingua START modifications */
//          drupal_set_message(join(' ', array($value, $location_info, $read_more)), $op);
            /** Lingua END modifications */
          break;
        case LINGUA_POTX_STATUS_CLI:
            /** Lingua START modifications */
          if ( defined ( 'STDERR' ) && defined ( 'STDOUT' ) ) {
            ...
            /** Lingua END modifications */
          break;
        case LINGUA_POTX_STATUS_SILENT:
          if ($op == 'error') {
            $messages[] = join(' ', array($value, $location_info, $read_more));
          }
          break;
        case LINGUA_POTX_STATUS_STRUCTURED:
          if ($op == 'error') {
 ...
 *   POTX_STRING_BOTH.
 */
function lingua_potx_find_t_calls( $file, $save_callback, $function_name = 't', $string_mode = LINGUA_POTX_STRING_RUNTIME) {
  global $_potx_tokens, $_potx_lookup;
      ...
      if ($par == "(") {
        if (in_array($rig, array(")", ","))
          && (is_array($mid) && ($mid[0] == T_CONSTANT_ENCAPSED_STRING))) {
            ...
            $save_callback(lingua_potx_format_quoted_string( $mid[1]), LINGUA_POTX_CONTEXT_NONE, $file, $line, $string_mode);
        }
        else {
          // $function_name() found, but inside is something which is not a string literal.
          lingua_potx_marker_error($file, $line, $function_name, $ti, lingua_t('The first parameter to @function() should be a literal string. There should be no variables, concatenation, constants or other non-literal strings there.', array( '@function' => $function_name)), 'http://drupal.org/node/322732');
        }
      }
    }
  }
 ...
 *   POTX_STRING_BOTH.
 */
function lingua_potx_find_t_calls_with_context( $file, $save_callback, $function_name = '__', $string_mode = LINGUA_POTX_STRING_RUNTIME) {
  global $_potx_tokens, $_potx_lookup;
      ...
      if ($par == "(") {
        if (in_array($rig, array(")", ","))
          ...
          // By default, there is no context.
            /** Lingua START modifications */
            $domain = LINGUA_POTX_CONTEXT_NONE;
            $context = false;
			$text_plural = false;
          if ($rig == ',') {
          ...
          if ($domain !== LINGUA_POTX_CONTEXT_ERROR) {
            // Only save if there was no error in context parsing.
            $save_callback(lingua_potx_format_quoted_string( $mid[1]), $domain, @strval ( $context ), $file, $line, $string_mode, lingua_potx_format_quoted_string ( $text_plural ));
          }
          /** Lingua END modifications */
        }
        else {
 ...
 *   Callback function used to save strings.
 */
function lingua_potx_find_watchdog_calls( $file, $save_callback) {
  global $_potx_tokens, $_potx_lookup;
      ...
      if ($par == '(') {
        ...
        if (in_array($rig, array(')', ',')) && $comma == ','
          ...
          && (is_array($message) && ($message[0] == T_CONSTANT_ENCAPSED_STRING))) {
            ...
            $save_callback(lingua_potx_format_quoted_string( $mtype[1]), LINGUA_POTX_CONTEXT_NONE, $file, $line);
            $save_callback(lingua_potx_format_quoted_string( $message[1]), LINGUA_POTX_CONTEXT_NONE, $file, $line);
        }
        else {
          // watchdog() found, but inside is something which is not a string literal.
          lingua_potx_marker_error($file, $line, 'watchdog', $ti, lingua_t('The first two watchdog() parameters should be literal strings. There should be no variables, concatenation, constants or even a t() call there.'), 'http://drupal.org/node/323101');
        }
      }
    }
  }
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_find_format_plural_calls( $file, $save_callback, $api_version = LINGUA_POTX_API_CURRENT) {
  global $_potx_tokens, $_potx_lookup;
  ...
  if (isset($_potx_lookup['format_plural'])) {
    foreach ($_potx_lookup['format_plural'] as $ti) {
      ...
      if ($par1 == "(") {
        ...
        if (($comma2 == ',') && ($par2 == ')' || ($par2 == ',' && $api_version > LINGUA_POTX_API_5)) &&
          (is_array($singular) && ($singular[0] == T_CONSTANT_ENCAPSED_STRING)) &&
          (is_array($plural) && ($plural[0] == T_CONSTANT_ENCAPSED_STRING))) {
          // By default, there is no context.
          $context = LINGUA_POTX_CONTEXT_NONE;
          if ($par2 == ',' && $api_version > LINGUA_POTX_API_6) {
            // If there was a comma after the plural, we need to look forward
            // to try and find the context.
            $context = lingua_potx_find_context($ti, $tn + 5, $file, 'format_plural');
          }
          if ($context !== LINGUA_POTX_CONTEXT_ERROR) {
            // Only save if there was no error in context parsing.
            $save_callback(
              lingua_potx_format_quoted_string( $singular[1]) ."\0". lingua_potx_format_quoted_string( $plural[1]),
              $context,
              $file,
              $line
            );
          ...
          lingua_potx_marker_error($file, $line, "format_plural", $ti, lingua_t('In format_plural(), the singular and plural strings should be literal strings. There should be no variables, concatenation, constants or even a t() call there.'), 'http://drupal.org/node/323072');
        }
      }
    }
  }
 ...
 *   Callback function used to save strings.
 */
function lingua_potx_find_perm_hook( $file, $filebase, $save_callback) {
  global $_potx_tokens, $_potx_lookup;
  ...
  if (isset($_potx_lookup[$filebase .'_perm'])) {
    ...
    if ($filebase == 'node') {
      ...
      foreach ($nodeperms as $item) {
        ...
        // support context.
        $save_callback($item, LINGUA_POTX_CONTEXT_NONE, $file, $line);
      }
    }
    else {
      $count = 0;
      foreach ($_potx_lookup[$filebase .'_perm'] as $ti) {
        ...
        while (is_array($_potx_tokens[$tn]) || $_potx_tokens[$tn] != '}') {
          if (is_array($_potx_tokens[$tn]) && $_potx_tokens[$tn][0] == T_CONSTANT_ENCAPSED_STRING) {
            // hook_perm() is only ever found on a Drupal system which does not
            // support context.
            $save_callback(lingua_potx_format_quoted_string( $_potx_tokens[ $tn][1]), LINGUA_POTX_CONTEXT_NONE, $file, $_potx_tokens[ $tn][2]);
            $count++;
          }
          $tn++;
        }
      ...
      if (!$count) {
        lingua_potx_status('error', lingua_t('%hook should have an array of literal string permission names.', array( '%hook' => $filebase .'_perm()')), $file, NULL, NULL, 'http://drupal.org/node/323101');
      }
    }
  }
}
 ...
 */
function lingua_potx_find_end_of_function( $here) {
  global $_potx_tokens;
 ...
 */
function lingua_potx_skip_args( $here) {
  global $_potx_tokens;
    ...
    if ($com == ',' && $arr[1] == 'array' && $par == '(') {
      ...
      if ($nesting == 0) {
        ...
        if (is_array($arw) && $arw[1] == '=>' && is_array($str) && $str[0] == T_CONSTANT_ENCAPSED_STRING) {
          return lingua_potx_format_quoted_string( $str[1]);
        }
        else {
          list($type, $string, $line) = $_potx_tokens[$ti];
          // @todo: fix error reference.
          lingua_potx_marker_error($file, $line, $function_name, $tf, lingua_t('The context element in the options array argument to @function() should be a literal string. There should be no variables, concatenation, constants or other non-literal strings there.', array( '@function' => $function_name)), 'http://drupal.org/node/322732');
          // Return with error.
          return LINGUA_POTX_CONTEXT_ERROR;
        }
      }
      else {
        // Did not found 'context' key in $options array.
        return LINGUA_POTX_CONTEXT_NONE;
      }
    }
  }
 ...
 *   Callback function used to save strings.
 */
function lingua_potx_find_menu_hooks( $file, $filebase, $save_callback) {
  global $_potx_tokens, $_potx_lookup;
  ...
  foreach ($hooks as $hook) {
    if (isset($_potx_lookup[$filebase . $hook]) && is_array($_potx_lookup[$filebase . $hook])) {
      ...
      foreach ($_potx_lookup[$filebase . $hook] as $ti) {
        $end = lingua_potx_find_end_of_function($ti);
        $tn = $ti;
        while ($tn < $end) {
          ...
          if ($_potx_tokens[$tn][0] == T_CONSTANT_ENCAPSED_STRING && in_array($_potx_tokens[$tn][1], $keys) && $_potx_tokens[$tn+1][0] == T_DOUBLE_ARROW) {
            if ($_potx_tokens[$tn+2][0] == T_CONSTANT_ENCAPSED_STRING) {
              ...
              $save_callback(
                lingua_potx_format_quoted_string( $_potx_tokens[ $tn+2][1]),
                LINGUA_POTX_CONTEXT_NONE,
                $file,
                $_potx_tokens[$tn+2][2]
              );
              ...
              lingua_potx_status('error', lingua_t('Invalid menu %element definition found in %hook. Title and description keys of the menu array should be literal strings.', array( '%element' => $_potx_tokens[ $tn][1], '%hook' => $filebase . $hook .'()')), $file, $_potx_tokens[ $tn][2], NULL, 'http://drupal.org/node/323101');
            }
          }
          ...
          if (is_string($_potx_tokens[$tn]) && $_potx_tokens[$tn] == '[' && $_potx_tokens[$tn+1][0] == T_CONSTANT_ENCAPSED_STRING && in_array($_potx_tokens[$tn+1][1], $keys) && is_string($_potx_tokens[$tn+2]) && $_potx_tokens[$tn+2] == ']') {
            if (is_string($_potx_tokens[$tn+3]) && $_potx_tokens[$tn+3] == '=' && $_potx_tokens[$tn+4][0] == T_CONSTANT_ENCAPSED_STRING) {
              // We cannot export menu item context.
              $save_callback(
                lingua_potx_format_quoted_string( $_potx_tokens[ $tn+4][1]),
                LINGUA_POTX_CONTEXT_NONE,
                $file,
                $_potx_tokens[$tn+4][2]
              );
              ...
              lingua_potx_status('error', lingua_t('Invalid menu %element definition found in %hook. Title and description keys of the menu array should be literal strings.', array( '%element' => $_potx_tokens[ $tn+1][1], '%hook' => $filebase . $hook .'()')), $file, $_potx_tokens[ $tn+1][2], NULL, 'http://drupal.org/node/323101');
            }
          }
          $tn++;
        }
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_find_language_names( $file, $save_callback, $api_version = LINGUA_POTX_API_CURRENT) {
  global $_potx_tokens, $_potx_lookup;

  foreach ($_potx_lookup[$api_version > LINGUA_POTX_API_5 ? '_locale_get_predefined_list' : '_locale_get_iso639_list'] as $ti) {
  ...
  while ($ti < $end) {
    ...
    $save_callback(lingua_potx_format_quoted_string( $_potx_tokens[ $ti][1]), LINGUA_POTX_CONTEXT_NONE, $file, $_potx_tokens[ $ti][2]);
  }
}
 ...
 */
function lingua_potx_find_version_number( $code, $file, $version_callback) {
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_add_date_strings( $file, $save_callback, $api_version = LINGUA_POTX_API_CURRENT) {
  for ($i = 1; $i <= 12; $i++) {
    $stamp = mktime(0, 0, 0, $i, 1, 1971);
    if ($api_version > LINGUA_POTX_API_6) {
      // From Drupal 7, long month names are saved with this context.
      $save_callback(date("F", $stamp), 'Long month name', $file);
    }
    elseif ($api_version > LINGUA_POTX_API_5) {
      // Drupal 6 uses a little hack. No context.
      $save_callback('!long-month-name '. date("F", $stamp), LINGUA_POTX_CONTEXT_NONE, $file);
    }
    else {
      // Older versions just accept the confusion, no context.
      $save_callback(date("F", $stamp), LINGUA_POTX_CONTEXT_NONE, $file);
    }
    // Short month names lack a context anyway.
    $save_callback(date("M", $stamp), LINGUA_POTX_CONTEXT_NONE, $file);
  }
  for ($i = 0; $i <= 7; $i++) {
    $stamp = $i * 86400;
    $save_callback(date("D", $stamp), LINGUA_POTX_CONTEXT_NONE, $file);
    $save_callback(date("l", $stamp), LINGUA_POTX_CONTEXT_NONE, $file);
  }
  $save_callback('am', LINGUA_POTX_CONTEXT_NONE, $file);
  $save_callback('pm', LINGUA_POTX_CONTEXT_NONE, $file);
  $save_callback('AM', LINGUA_POTX_CONTEXT_NONE, $file);
  $save_callback('PM', LINGUA_POTX_CONTEXT_NONE, $file);
}
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_add_format_interval_strings( $file, $save_callback, $api_version = LINGUA_POTX_API_CURRENT) {
  $components = array(
    '1 year' => '@count years',
    '1 week' => '@count weeks',
    '1 day'  => '@count days',
  ...
  foreach ($components as $singular => $plural) {
    ...
    $save_callback($singular ."\0". $plural, LINGUA_POTX_CONTEXT_NONE, $file);
  }
}
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_add_default_region_names( $file, $save_callback, $api_version = LINGUA_POTX_API_CURRENT) {
  $regions = array(
    'left' => 'Left sidebar',
    'right' => 'Right sidebar',
    'content' => 'Content',
  ...
  foreach ($regions as $region) {
    ...
    $save_callback($region, LINGUA_POTX_CONTEXT_NONE, $file);
  }
}
 ...
 *   Drupal API version to work with.
 */
function lingua_potx_find_info_file_strings( $file_path, $file_name, $save_callback, $api_version = LINGUA_POTX_API_CURRENT) {
  $info = array();

  if (file_exists($file_path)) {
    $info = $api_version > LINGUA_POTX_API_5 ? lingua_drupal_parse_info_file($file_path) : parse_ini_file($file_path);
  }
  ...
  foreach (array('name', 'description', 'package') as $key) {
    if (isset($info[$key])) {
      ...
      $save_callback(addcslashes($info[$key], "\0..\37\\\""), LINGUA_POTX_CONTEXT_NONE, $file_name);
    }
  }
  ...
  if (isset($info['regions']) && is_array($info['regions'])) {
    foreach ($info['regions'] as $region => $region_name) {
      // No context support for .info file strings.
      $save_callback(addcslashes($region_name, "\0..\37\\\""), LINGUA_POTX_CONTEXT_NONE, $file_name);
    }
  }
}
 ...
 */
function lingua_potx_parse_js_file( $code, $file, $save_callback) {
  $js_string_regex = '(?:(?:\'(?:\\\\\'|[^\'])*\'|"(?:\\\\"|[^"])*")(?:\s*\+\s*)?)+';
  ...
  if (isset($t_matches) && count($t_matches)) {
    foreach ($t_matches as $match) {
      ...
      $save_callback(lingua_potx_parse_js_string( $match[1]), LINGUA_POTX_CONTEXT_NONE, $file, 0);
    }
  }
  ...
  if (isset($plural_matches) && count($plural_matches)) {
    foreach ($plural_matches as $index => $match) {
      ...
      // @todo: figure out how to parse out context, once Drupal supports it.
      $save_callback(
        lingua_potx_parse_js_string( $match[1]) ."\0". lingua_potx_parse_js_string( $match[2]),
        LINGUA_POTX_CONTEXT_NONE,
        $file,
        0
      );
  ...
  if (isset($faulty_matches) && count($faulty_matches)) {
    foreach ($faulty_matches as $index => $match) {
      $message = ($match[1] == 't') ? lingua_t('Drupal.t() calls should have a single literal string as their first parameter.') : lingua_t('The singular and plural string parameters on Drupal.formatPlural() calls should be literal strings, plural containing a @count placeholder.');
      lingua_potx_status('error', $message, $file, NULL, $match[0], 'http://drupal.org/node/323109');
    }
  }
}
 ...
 */
function lingua_potx_parse_js_string( $string) {
  return lingua_potx_format_quoted_string(implode('', preg_split('~(?<!\\\\)[\'"]\s*\+\s*[\'"]~s', $string)));
}
 ...
 */
function lingua_potx_explore_dir( $path = '', $basename = '*', $api_version = LINGUA_POTX_API_CURRENT, $skip_self = FALSE) {
  ...
  if ($api_version > LINGUA_POTX_API_5) {
    $extensions[] = 'js';
  }
  $files = array();
  ...
  if (is_array($dirs)) {
    foreach ($dirs as $dir) {
      if (!preg_match("!(^|.+/)(CVS|\.svn|\.git|tests)$!", $dir)) {
        $files = array_merge($files, lingua_potx_explore_dir("$dir/", $basename));
      }
    }
  }
 ...
 */
function lingua_potx_save_version( $value = NULL, $file = NULL) {
 ...
 *   or POTX_STRING_BOTH.
 */
function lingua_potx_save_string( $value = NULL, $context = NULL, $file = NULL, $line = 0, $string_mode = LINGUA_POTX_STRING_RUNTIME) {
  ...
  if (isset($value)) {
    ...
    if (empty($check_empty)) {
      lingua_potx_status('error', lingua_t('Empty string attempted to be localized. Please do not leave test code for localization in your source.'), $file, $line);
      return;
    }

    switch ($string_mode) {
      case LINGUA_POTX_STRING_BOTH:
      ...
      case LINGUA_POTX_STRING_RUNTIME:
        // Mark runtime strings as duplicates of installer strings if
        // the string was both recorded in the runtime and in the installer.
        $_potx_strings[$value][$context][$file][] = $line . ($string_mode == LINGUA_POTX_STRING_BOTH ? ' (dup)' : '');
        break;
      case LINGUA_POTX_STRING_INSTALLER:
        $_potx_install[$value][$context][$file][] = $line;
        break;
    }
  }
  else {
    return ($string_mode == LINGUA_POTX_STRING_RUNTIME ? $_potx_strings : $_potx_install);
  }
}

if (!function_exists('lingua_t')) {
  // If invoked outside of Drupal, t() will not exist, but
  // used to format the error message, so we provide a replacement.
  function lingua_t( $string, $args = array()) {
    return strtr($string, $args);
  }
}

if (!function_exists('lingua_drupal_parse_info_file')) {
  ...
  function lingua_drupal_parse_info_file( $filename) {
    $info = array();
